...
function remove_from_cart($id)
{
	...
	for ($i = 0; $i < $length; $i++)
		if ($_SESSION['cart'][$i] == $id) $index = $i;
		
	// сдвигаем
	for ($i = $index; $i < $length - 1; $i++)
		$_SESSION['cart'][$i] = $_SESSION['cart'][$i + 1];
	
	unset($_SESSION['cart'][$length - 1]);
}
...
function is_in_cart($id)
{
	...
	for ($i = 0; $i < count($_SESSION['cart']); $i++)
		if ($_SESSION['cart'][$i] == $id) return true;
	
	return false;
}


function count_goods_in_cart()
{
...
function remove_from_cart($id, $count)
{
	...
	if (!isset($_SESSION['cart'][0])) clear_cart();
}
...
function calculate_goods_count()
{
	$count = 0;
	...
	for ($i = 0; ; $i++)
	{
		...
		$full_cost += $_SESSION['cart'][$i]['count'];
	}
