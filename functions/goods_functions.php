...
{
	$query = "SELECT img FROM goods WHERE id = '$id'";
...
function get_top_goods($count)
{
	$data = get_on_main_goods();
	
	if ($data) $count -= count($data);
	
	return get_goods_by_query("SELECT * FROM goods ORDER BY popularity LIMIT $count", 1);
}
...
function get_on_main_goods()
{
    return get_goods_by_query("SELECT * FROM goods WHERE on_main = '1'", 1);
}
