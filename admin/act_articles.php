...
else if ($mode == 'by_id')
{
	...
	if ($data['img'] == '/gallery/articles/no_image.jpg')
		$img = '<b>Нет изображения!</b>';
	else 
		$img = '
	...
	<form method="post" action="" enctype="multipart/form-data">
		...
		Картинка предпросмотра <span>(размер 150x200, будет показана на главной странице рядом с названием статьи)</span>:<br><br>
		<?=$img?><br>
		Выберите файл для загрузки <span>(меньше 10МБ, меньше 5000x5000 точек)</span>:<br> 
        ...
        Популярность <span>(чем больше число, тем больше популярность)</span>:<br>
		...
		Показывать на главной странице? <span>(если '1', то статья будет отображаться на главной странице, иначе - '0')</span>:<br>
        <input type="text" name="on_main" value="<?=$data['on_main']?>" maxlength="200" id="mini_text_input"/><br><br>
    ...
    print_add_comment_form_for_admin('article_id', $id);
}
?>